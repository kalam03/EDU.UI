...
@Component({ selector: 'app-fees-list', standalone: true, imports: [CommonModule, FormsModule, RouterLink, DataTableComponent, SearchableDropdownComponent], templateUrl: './fees-list.component.html', styleUrl: './fees-list.component.scss' })
export class FeesListComponent implements OnInit {
  private feeService = inject(FeePaymentService);
  private router = inject(Router);
  fees: FeePayment[] = []; filtered: FeePayment[] = []; loading = true; filterStatus = '';
  statusOptions: DropdownOption[] = [
    { value: '', label: 'All Status' }, { value: 'Paid', label: 'Paid' },
    { value: 'Unpaid', label: 'Unpaid' }, { value: 'Partial', label: 'Partial' }
  ];
  onStatusSelect(val: number | string | null): void { this.filterStatus = (val ?? '') as string; this.applyFilter(); }
  columns: TableColumn[] = [
    { key: 'paymentId',    label: '#' },
  ...
  applyFilter(): void { this.filtered = this.filterStatus ? this.fees.filter(f => f.status === this.filterStatus) : this.fees; }
